...
<2> CUDA_VISIBLE_DEVICES=1,3 python -m torch.distributed.launch --nproc_per_node=2 --use_env train_multi_gpu_using_launch.py
...
from multi_gpus_utils.distributed_utils import init_distributed_mode, dist, cleanup
from multi_gpus_utils.train_eval_utils import train_step, val_step
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        mean_loss = train_step(net=model,
                               optimizer=optimizer,
                               data_loader=train_loader,
                               device=device,
                               epoch=epoch, scalar=scalar)

        scheduler.step()

        sum_num = val_step(net=model,
        ...
        if rank == 0:
            ...
            torch.save(model.module.state_dict(), "./weights/model.pth")
...
if __name__ == '__main__':
    ...
    # 数据集所在根目录
    parser.add_argument('--data-path', type=str, default="/mnt/d/flower_data")
