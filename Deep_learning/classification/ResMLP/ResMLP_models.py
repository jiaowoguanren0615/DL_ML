...
class resmlp_models(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12, drop_rate=0.,
        ...
        self.patch_embed = Patch_layer(
            img_size=img_size, patch_size=patch_size, in_c=int(in_chans), embed_dim=embed_dim)
        num_patches = self.patch_embed.num_patches

        dpr = [drop_path_rate for i in range(depth)] # equal: [drop_path_rate] * depth

        # dropout_rate = np.linspace(0, drop_rate, depth) --- > dropout_rate[i]
...
def resmlp_12(num_classes, pretrained=False, dist=False, **kwargs):
    model = resmlp_models(num_classes=num_classes,
...
def resmlp_24(num_classes, pretrained=False, dist=False, dino=False, **kwargs):
    model = resmlp_models(num_classes=num_classes,
...
def resmlp_36(num_classes, pretrained=False, dist=False, **kwargs):
    model = resmlp_models(num_classes=num_classes,
...
def resmlpB_24(num_classes, pretrained=False, dist=False, in_22k=False, **kwargs):
    model = resmlp_models(num_classes=num_classes,
