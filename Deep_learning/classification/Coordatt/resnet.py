...
from coordatt import CoordAttention
from utils import get_net_summary


class BasicBlock(nn.Module):
    expansion = 1  # 表示在每个残差结构块内部 channel的变化 (ResNet18和ResNet34中每个结构块中的channel都是一样的)
...
class ResNet(nn.Module):
    def __init__(self, block, block_list, num_classes, groups=1, width_per_group=64) -> None:
        ...
        self.phi = CoordAttention(2048, 2048)
...
net = ResNet(BottleNeck, block_list=[3, 4, 6, 3], num_classes=5)
# get_net_summary(net, input_size=(1, 3, 224, 224))